...
@jesthon.on(events.NewMessage(outgoing=True, pattern=r"\.صيد (.*)"))
async def _(event):
    if ispay2[0] == "yes":
        isclaim.clear()
        isclaim.append("on")
        ...
        @jesthon.on(events.NewMessage(outgoing=True, pattern=r"\.حالة الصيد"))
        async def _(event):
            if ispay2[0] == "yes":
                if "on" in isclaim:
...
async def _(event):
    if ispay2[0] == "yes":
        ...
        if msg[0] == "تثبيت":  # تثبيت عدد يوزر قناه
            ...
            @jesthon.on(events.NewMessage(outgoing=True, pattern=r"\.condition Attract"))
            async def _(event):
                if "on" in isauto:
